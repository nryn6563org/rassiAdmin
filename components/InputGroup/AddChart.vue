<template>
  <div class="addChart">
    <label for="addChartInput">
      <input type="text" name="" id="addChartInput" class="custom-text-input" autocomplete="off" :placeholder="searchLabel" :value="value" @input="onInput" @focus="isOpen = true" @keydown.down.prevent="onArrowDown" @keydown.up.prevent="onArrowUp" @keydown.enter.prevent="onEnter" />
      <button class="btn013 round" type="submit">차트추가</button>
    </label>

    <ul v-if="isOpen && filteredItems.length > 0" class="suggestions-list">
      <li v-for="(item, index) in filteredItems" :key="index" :class="{ 'is-active': index === arrowCounter }" @click="selectItem(item)">
        {{ item }}
      </li>
    </ul>
  </div>
...
export default {
  ...
  computed: {
    ...
    filteredItems() {
      ...
      return this.items.filter(item => item.toLowerCase().includes(this.value.toLowerCase()))
    }
  },
