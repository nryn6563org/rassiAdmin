  ...
  <div class="table_w">
    <div class="table" ref="tableContainer">
      <table :style="{ width: tableStyleWidth }">
        ...
        <GroupTbody :tbodyLists="bodyData" />
      </table>
    </div>
...
import GroupTbody from '@/components/UM/Group/GroupTbody.vue'
...
export default {
  ...
  data() {
    return {
      isAllChecked: false,
      headerData: ['', 'No', '소속명', '소속분류', '배정인원', '등록일시', '관리', '삭제'],
      // 4. 원본 컬럼 너비 정의 (고정값 관리)
      originColWidths: [60, 90, 660, 200, 120, 200, 130, 130],
      ...
      bodyData: Array.from({ length: 10 }, (_, i) => {
        const num = i + 1
        const authOptions = ['투자자문사', '유사투자자문업자', '투자일임사', '자산운용사']

        return {
          checked: false,
          ListNumber: num,
          AffiliationName: '2025년 현대로템의 특별 매매 코멘트로 전격 분석 내용을 전격으로 파헤쳐보는 시간을 갖겠습니다.',
          PeopleCount: 12,
          Affiliation: authOptions[Math.floor(Math.random() * authOptions.length)],
