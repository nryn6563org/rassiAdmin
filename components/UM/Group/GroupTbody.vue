    ...
    <tr v-for="(tbodyList, tbodyListIndex) in tbodyLists" :key="tbodyListIndex">
      ...
      <td>
        <div class="affiliationName">
          <span>{{ tbodyList.AffiliationName }}</span>
        </div>
      </td>
      <!-- 소속명 -->
      <td>
      ...
      <td>
        <div class="peopleCount">
          <span>{{ tbodyList.PeopleCount }}</span>
        </div>
      </td>
      <!-- 배정인원 -->
      <td>
      ...
      <td>
        <div class="delete">
          <button @click="handleModalClick($event)" class="btn006 btn-mid groupDlt">
            <span>삭제</span>
          </button>
        </div>
      </td>
...
<script>
import GroupDelete from '@/components/Modal/GroupDeleteModal.vue'
...
export default {
  ...
  methods: {
    ...
    handleModalClick(event) {
      ...
      if (button) {
        if (button.classList.contains('groupDlt')) {
          // 삭제
