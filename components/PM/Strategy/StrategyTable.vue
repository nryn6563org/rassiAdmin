  ...
  <div class="table_w">
    <div class="table" ref="tableContainer">
      <table :style="{ width: tableStyleWidth }">
        ...
        <StrategyTbody :tbodyLists="bodyData" />
      </table>
    </div>
...
import StrategyTbody from '@/components/PM/Strategy/StrategyTbody.vue'
...
export default {
  ...
  data() {
    return {
      isAllChecked: false,
      headerData: ['', 'No', '제목', '공개범위', '관련종목', '대표이미지', '작성자', '등록일시', '관리', '삭제'],
      // 4. 원본 컬럼 너비 정의 (고정값 관리)
      originColWidths: [60, 90, 460, 120, 120, 160, 120, 202, 130, 130],
      ...
      bodyData: Array.from({ length: 10 }, (_, i) => {
        ...
        return {
          ...
          PublicScope: Math.random() > 0.5 ? '회원전용' : '전부공개',
          RelatedStock: 10,
          PrevImage: require('~/assets/img/PM/prevImg.png'),
          Author: '홍길동',
