<template>
  <div id="Sell">
    <div class="suggestion">
      <section>
        <strong>선택된 종목명</strong>
        <div class="itemSearch">
          <label for="itemSearchInput">
            <input type="text" placeholder="" value="씨제이바이오사이언스 (311690)" id="itemSearchInput" class="custom-text-input" />
          </label>
        </div>

        <div class="txt">
          <div>
            <span>현재가 (등락률) :</span>
            <div class="itemPrice">
              <span class="prc">347,500</span>
              <span class="per dn">(-8.33%)</span>
              <span class="date"> 2025.11.28 15:23 </span>
              <button class="reset">
                <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                  <rect width="21" height="21" fill="url(#pattern0_779_12642)" />
                  <defs>
                    <pattern id="pattern0_779_12642" patternContentUnits="objectBoundingBox" width="1" height="1">
                      <use xlink:href="#image0_779_12642" transform="scale(0.02)" />
                    </pattern>
                    <image
                      id="image0_779_12642"
                      width="50"
                      height="50"
                      preserveAspectRatio="none"
                      xlink:href="data:image/png;base64,"
                    />
                  </defs>
                </svg>

                <span>실시간 현재가 조회</span>
              </button>
            </div>
          </div>
          <!-- 현재가(등락률) -->

          <div>
            <span>나의수익 :</span>
            <div class="itemPrice">
              <span class="prc">34,500</span>
              <span class="per up">(+3.33%)</span>
              <span class="date"> 2025.11.28 15:23 - 12일보유</span>
            </div>
          </div>
          <!-- 나의수익 -->
          <span>※ 등록 시 현재가는 자동 업데이트 됩니다.</span>
        </div>
      </section>

      <section>
        <div class="check_comment">
          <div>
            <div class="check">
              <label for="checkBuyComment" class="custom-checkbox-label">
                <input type="checkbox" id="checkBuyComment" class="custom-checkbox-input" v-model="useBuyComment" />
                <span class="custom-checkbox-box"></span>
                <span class="custom-checkbox-txt">매도 코멘트 :</span>
              </label>
            </div>
            <div class="comment">
              <input type="text" class="custom-text-input" placeholder="매도 알림과 제시될 한 줄 코멘트를 작성해 주세요." style="width: 708px" :disabled="!useBuyComment" />
              <p class="caution">*최소 5자, 최대 50자 범위로 입력해 주세요.</p>
            </div>
          </div>
        </div>
        <div class="txt">
          <span>※ 기 입력된 매도 코멘트가 있더라도 위 코멘트로 발송되며, 매도처리 됩니다.</span>
        </div>
      </section>
    </div>

    <div class="modal-footer">
      <div class="button-group">
        <button class="btn-large btn009" @click="handleClose">
          <span>닫기</span>
        </button>

        <button class="btn-large btn002" @click="handleConfirm">
          <span>실시간 매도 추천</span>
        </button>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      // 1. 추천 매수가 예약 설정 관련
      isBuyRsvMode: false, // 상승 구간 구매하기 체크 여부
      buyStandard: 'price', // 금액 vs 수익률 (매수)

      // 3. 목표가/손절가 설정 관련
      isTargetStopMode: false, // 자동 매도 알림 설정 체크 여부
      targetStandard: 'price', // 금액 vs 수익률 (매도)

      // 4. 코멘트 관련
      useBuyComment: true,
      useTargetComment: true,
      useStopComment: false
    }
  },
  computed: {
    calculatedDays() {
      // startDate와 endDate를 계산해서 일수를 반환하는 로직 (예시)
      return 4
    }
  },
  methods: {
    // 닫기 버튼 클릭 시 실행
    handleClose() {
      // layouts/default.vue에 정의된 리스너에게 '닫아달라'는 신호를 보냄
      this.$nuxt.$emit('close-global-modal')
    },

    // 실시간 매수 추천 버튼 클릭 시 실행
    handleConfirm() {
      // 1. 유효성 검사 또는 API 전송 로직 작성
      // if ( ... ) { alert('입력을 확인해주세요'); return; }
      console.log('실시간 매수 추천 전송 로직 수행')

      // 2. 로직 수행 후 모달 닫기
      this.handleClose()
    }
  }
}
</script>

<style scoped>
.suggestion{
  @apply mt-0
}
</style>
