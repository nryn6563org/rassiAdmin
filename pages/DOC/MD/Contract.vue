    ...
    try {
      if (process.server) {
        ...
        contractHtml = await new Promise((resolve) => {
          https.get('https://tradingpoint.co.kr/trading/contract/alphaquant.do', (res) => {
            let data = ''
        ...
        const response = await window.fetch('https://tradingpoint.co.kr/trading/contract/alphaquant.do')
        contractHtml = await response.text()
      }
    } catch (err) {
      console.error('Failed to load HTML', err)
      contractHtml = '<p>문서 로드 오류</p>'
    }
    return { contractHtml }
  },
  methods: {
